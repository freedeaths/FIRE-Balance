...
- **收支项目**：详细的收入和支出项目，包括起止年龄、增长率等；也支持用“阶段”快速绑定年龄段（仅用于配置便利，计算仍以起止年龄为准）
  - 阶段 1：当前年龄 ～ 目标 FIRE 年龄
  - 阶段 2：目标 FIRE 年龄 + 1 ～ 法定退休年龄
  - 阶段 3：法定退休年龄 + 1 ～ 预期寿命
- 说明：系统以“年”为计算周期，阶段之间不重叠；收支项支持选择连续阶段组合（如 阶段1-2、阶段2-3、阶段1-3）；一次性事件请使用自定义起止年龄
