...
export function createBlackSwanEvents(
  ...
  return [
    ...
    // Positive events
    new InheritanceEvent([
      Math.max(current_age, 30),
      Math.min(life_expectancy, 80),
    ]),
    new InvestmentWindfallEvent([
      Math.max(current_age, 25),
      Math.min(life_expectancy, 70),
    ]),
  ];
}
