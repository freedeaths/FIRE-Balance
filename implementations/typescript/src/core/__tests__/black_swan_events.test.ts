...
describe('BlackSwanEventImplementations', () => {
  test('FinancialCrisisEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.016);
    expect(event.duration_years).toBe(2);
    expect(event.recovery_factor).toBe(0.8);
    expect(event.age_range).toEqual([18, 100]);
    expect(event.income_impact).toBe(-0.4);
    expect(event.expense_impact).toBe(0.0);
  });
  ...
  test('EconomicRecessionEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.024);
    expect(event.duration_years).toBe(1);
    expect(event.recovery_factor).toBe(0.7);
    expect(event.age_range).toEqual([18, 100]);
    expect(event.income_impact).toBe(-0.25);
    expect(event.expense_impact).toBe(0.0);
  });
  ...
  test('MarketCrashEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.012);
    expect(event.duration_years).toBe(1);
    expect(event.recovery_factor).toBe(0.6);
    expect(event.age_range).toEqual([18, 100]);
    expect(event.income_impact).toBe(-0.3);
    expect(event.expense_impact).toBe(0.0);
  });
  ...
  test('HyperinflationEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.004);
    expect(event.duration_years).toBe(3);
    expect(event.recovery_factor).toBe(0.9);
    expect(event.age_range).toEqual([18, 100]);
    expect(event.income_impact).toBe(0.0);
    expect(event.expense_impact).toBe(0.5);
  });
  ...
  test('UnemploymentEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.02);
    expect(event.duration_years).toBe(1);
    expect(event.recovery_factor).toBe(0.5);
    expect(event.age_range).toEqual([22, 65]);
    expect(event.income_impact).toBe(-0.8);
    expect(event.expense_impact).toBe(0.0);
  });
  ...
  test('IndustryCollapseEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.008);
    expect(event.duration_years).toBe(3);
    expect(event.recovery_factor).toBe(0.7);
  ...
  test('UnexpectedPromotionEvent properties', () => {
    ...
    expect(event.recovery_factor).toBe(1.0);
  ...
  test('MajorIllnessEvent properties', () => {
    ...
    expect(event.income_impact).toBe(-0.3);
    expect(event.expense_impact).toBe(0.8);
  });
  ...
  test('LongTermCareEvent properties', () => {
    ...
    expect(event.income_impact).toBe(0.0);
    expect(event.expense_impact).toBe(1.2);
  });
  ...
  test('RegionalConflictEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.006);
    expect(event.duration_years).toBe(2);
    expect(event.recovery_factor).toBe(0.8);
    expect(event.age_range).toEqual([18, 100]);
    expect(event.income_impact).toBe(-0.2);
    expect(event.expense_impact).toBe(0.3);
  });
  ...
  test('GlobalWarEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.002);
    expect(event.duration_years).toBe(4);
    expect(event.recovery_factor).toBe(0.7);
    expect(event.age_range).toEqual([18, 100]);
    expect(event.income_impact).toBe(-0.5);
    expect(event.expense_impact).toBe(1.0);
  });
  ...
  test('EconomicSanctionsEvent properties', () => {
    ...
    expect(event.recovery_factor).toBe(0.8);
    expect(event.age_range).toEqual([18, 100]);
    expect(event.income_impact).toBe(-0.15);
    expect(event.expense_impact).toBe(0.2);
  });
  ...
  test('EnergyCrisisEvent properties', () => {
    ...
    expect(event.annual_probability).toBe(0.01);
    expect(event.duration_years).toBe(2);
    expect(event.recovery_factor).toBe(0.9);
    expect(event.age_range).toEqual([18, 100]);
    expect(event.income_impact).toBe(0.0);
    expect(event.expense_impact).toBe(0.4);
  });
  ...
  test('InheritanceEvent properties', () => {
    ...
    expect(event.income_impact).toBe(2.0);
    expect(event.expense_impact).toBe(0.0);
  });
  ...
  test('InvestmentWindfallEvent properties', () => {
    ...
    expect(event.income_impact).toBe(1.5);
    expect(event.expense_impact).toBe(0.0);
  });
});
...
describe('CreateBlackSwanEventsFactory', () => {
  ...
  test('all events have valid properties', () => {
    ...
    for (const event of events) {
      ...
      expect(event.annual_probability).toBeGreaterThanOrEqual(0);
      expect(event.annual_probability).toBeLessThanOrEqual(1);
      ...
      expect(event.recovery_factor).toBeGreaterThan(0);
      expect(event.recovery_factor).toBeLessThanOrEqual(1);
      ...
      if ('income_impact' in event) {
        expect(typeof (event as any).income_impact).toBe('number');
      }
      if ('expense_impact' in event) {
        expect(typeof (event as any).expense_impact).toBe('number');
      }
    }
  });
});
...
describe('EventCategorization', () => {
  ...
  test('identifies negative income events', () => {
    const negative_income_events = events.filter(e =>
      'income_impact' in e && e.income_impact < 0
    );
  ...
  test('identifies expense-increasing events', () => {
    const expense_events = events.filter(e =>
      'expense_impact' in e && e.expense_impact > 0
    );
  ...
  test('categorizes events by severity', () => {
    ...
    const high_severity = events.filter(e =>
      ('income_impact' in e && Math.abs(e.income_impact) >= 0.5) ||
      ('expense_impact' in e && Math.abs(e.expense_impact) >= 0.5)
    );
