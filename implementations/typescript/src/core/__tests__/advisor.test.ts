...
describe('FIREAdvisor', () => {
  ...
  test('early retirement recommendation', () => {
    ...
    const earlyRetirement = recommendations.find(r => r.type === 'early_retirement');
    if (earlyRetirement) {
  ...
  test('delayed retirement recommendation', () => {
    ...
    const delayedRetirement = recommendations.find(r => r.type === 'delayed_retirement');
  ...
  test('income adjustment recommendation', () => {
    ...
    const incomeAdjustment = recommendations.find(r => r.type === 'income_adjustment');
    if (incomeAdjustment) {
      expect(incomeAdjustment.is_achievable).toBe(true);
      expect(incomeAdjustment.params).toHaveProperty('suggested_increase_percent');
      expect(incomeAdjustment.params.suggested_increase_percent).toBeGreaterThan(0);
    }
  });
  ...
  test('expense reduction recommendation', () => {
    ...
    const expenseReduction = recommendations.find(r => r.type === 'expense_reduction');
    if (expenseReduction) {
      expect(expenseReduction.is_achievable).toBe(true);
      expect(expenseReduction.params).toHaveProperty('suggested_reduction_percent');
      expect(expenseReduction.params.suggested_reduction_percent).toBeGreaterThan(0);
    }
  });
  ...
  test('advisor with minimal data', () => {
