...
describe('Edge Cases', () => {
  ...
  test('deeply nested key access', () => {
    ...
    expect(i18n.t('level1.level2.level3.level4.missing')).toBe(
      'level1.level2.level3.level4.missing'
    );
  });
  ...
  test('variable substitution with special characters', () => {
    ...
    expect(i18n.t('special', { amount: '100.50', count: 3 })).toBe(
      'Price: $100.50 USD, quantity: 3 items'
    );
  });
  ...
  test('repeated variable substitution', () => {
    ...
    expect(i18n.t('repeated', { name: 'Alice' })).toBe(
      'Alice said: "Alice is great!", and Alice smiled.'
    );
  });
});
