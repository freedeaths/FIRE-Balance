...
describe('PlannerModels', () => {
  ...
  describe('Override model', () => {
    ...
    test('validate age range', () => {
      expect(() =>
        createOverride({ age: -1, item_id: 'test', value: new Decimal(0) })
      ).toThrow('Override age must be between 0 and 150');

      expect(() =>
        createOverride({ age: 151, item_id: 'test', value: new Decimal(0) })
      ).toThrow('Override age must be between 0 and 150');
    });

    test('validate item_id', () => {
      expect(() =>
        createOverride({ age: 30, item_id: '', value: new Decimal(0) })
  ...
  describe('PlannerData model', () => {
    ...
    test('update timestamp', () => {
      ...
      setTimeout(() => {
        ...
        expect(updatedData.updated_at.getTime()).toBeGreaterThan(
  ...
  describe('Configuration conversion', () => {
    ...
    test('roundtrip conversion preserves data', () => {
      ...
      expect(convertedData.user_profile?.birth_year).toBe(
        originalData.user_profile?.birth_year
      );
      expect(convertedData.income_items).toHaveLength(
        originalData.income_items.length
      );
      expect(convertedData.expense_items).toHaveLength(
        originalData.expense_items.length
      );
  ...
  describe('Stage transition utilities', () => {
    ...
    test('get next stage', () => {
      expect(getNextStage(PlannerStage.STAGE1_INPUT)).toBe(
        PlannerStage.STAGE2_ADJUSTMENT
      );
      expect(getNextStage(PlannerStage.STAGE2_ADJUSTMENT)).toBe(
        PlannerStage.STAGE3_ANALYSIS
      );
    ...
    test('is ready for stage', () => {
      ...
      expect(isReadyForStage(emptyData, PlannerStage.STAGE2_ADJUSTMENT)).toBe(
        false
      );
      expect(isReadyForStage(emptyData, PlannerStage.STAGE3_ANALYSIS)).toBe(
        false
      );
      ...
      expect(isReadyForStage(stage1Data, PlannerStage.STAGE2_ADJUSTMENT)).toBe(
        true
      );
      expect(isReadyForStage(stage1Data, PlannerStage.STAGE3_ANALYSIS)).toBe(
        false
      );
      ...
      const projectionData: AnnualProjectionRow[] = [
        {
          age: 34,
          year: 2024,
          total_income: new Decimal(80000),
          total_expense: new Decimal(50000),
        },
        {
          age: 35,
          year: 2025,
          total_income: new Decimal(81600),
          total_expense: new Decimal(50000),
        },
      ];
      ...
      expect(isReadyForStage(stage2Data, PlannerStage.STAGE2_ADJUSTMENT)).toBe(
        true
      );
      expect(isReadyForStage(stage2Data, PlannerStage.STAGE3_ANALYSIS)).toBe(
        true
      );
    });
  });
  ...
  describe('Edge cases and validation', () => {
    ...
    test('handle partial item data in conversion', () => {
      ...
      expect(
        plannerData.income_items[0].after_tax_amount_per_period.toNumber()
      ).toBe(0); // Default
