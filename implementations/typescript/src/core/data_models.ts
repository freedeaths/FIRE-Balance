...
export function validateAllocationSum(portfolio: PortfolioConfiguration): void {
  const total = portfolio.asset_classes.reduce((sum, asset) => sum.add(asset.allocation_percentage), new Decimal(0));
 ...
 */
export function createIncomeExpenseItem(data: Partial<IncomeExpenseItem> & {
  ...
  const item: IncomeExpenseItem = {
    id: data.id || uuidv4(),
    name: data.name || '',
    after_tax_amount_per_period: new Decimal(data.after_tax_amount_per_period || 0),
 ...
 */
export function createSimulationSettings(data: Partial<SimulationSettings> & {
  ...
  const settings: SimulationSettings = {
    ...
    confidence_level: new Decimal(data.confidence_level ?? DEFAULT_SIMULATION_SETTINGS.confidence_level),
    include_black_swan_events: data.include_black_swan_events ?? DEFAULT_SIMULATION_SETTINGS.include_black_swan_events,
    income_base_volatility: new Decimal(data.income_base_volatility ?? DEFAULT_SIMULATION_SETTINGS.income_base_volatility),
    income_minimum_factor: new Decimal(data.income_minimum_factor ?? DEFAULT_SIMULATION_SETTINGS.income_minimum_factor),
    expense_base_volatility: new Decimal(data.expense_base_volatility ?? DEFAULT_SIMULATION_SETTINGS.expense_base_volatility),
    expense_minimum_factor: new Decimal(data.expense_minimum_factor ?? DEFAULT_SIMULATION_SETTINGS.expense_minimum_factor),
  };
  ...
  if (settings.confidence_level.lt(0.5) || settings.confidence_level.gt(0.99)) {
    throw new Error(`confidence_level must be between 0.5 and 0.99, got ${settings.confidence_level.toString()}`);
  }

  // Validate volatility ranges (matches Python ge=0.0, le=1.0)
  if (settings.income_base_volatility.lt(0.0) || settings.income_base_volatility.gt(1.0)) {
    throw new Error(`income_base_volatility must be between 0.0 and 1.0, got ${settings.income_base_volatility.toString()}`);
  }
  if (settings.expense_base_volatility.lt(0.0) || settings.expense_base_volatility.gt(1.0)) {
    throw new Error(`expense_base_volatility must be between 0.0 and 1.0, got ${settings.expense_base_volatility.toString()}`);
  }

  // Validate minimum factors (matches Python ge=0.01, le=1.0 for income, ge=0.1, le=1.0 for expense)
  if (settings.income_minimum_factor.lt(0.01) || settings.income_minimum_factor.gt(1.0)) {
    throw new Error(`income_minimum_factor must be between 0.01 and 1.0, got ${settings.income_minimum_factor.toString()}`);
  }
  if (settings.expense_minimum_factor.lt(0.1) || settings.expense_minimum_factor.gt(1.0)) {
    throw new Error(`expense_minimum_factor must be between 0.1 and 1.0, got ${settings.expense_minimum_factor.toString()}`);
  }
