...
export function createOverride(data: Partial<Override> & { value?: number | Decimal }): Override {
  ...
  if (override.age < 0 || override.age > 150) {
    throw new Error(`Override age must be between 0 and 150, got ${override.age}`);
  }
 ...
 */
export function createPlannerData(data: Partial<PlannerData> = {}): PlannerData {
  const now = new Date();
  ...
  return {
    ...
    simulation_settings: data.simulation_settings ?? createSimulationSettings({}),
  };
}
 ...
 */
export function createPlannerConfigV1(data: Partial<PlannerConfigV1> = {}): PlannerConfigV1 {
  return {
...
export function configToPlannerData(config: PlannerConfigV1): PlannerData {
  ...
  const simulation_settings = createSimulationSettings(config.simulation_settings);
 ...
 */
export function isReadyForStage(data: PlannerData, stage: PlannerStage): boolean {
  switch (stage) {
    case PlannerStage.STAGE1_INPUT:
