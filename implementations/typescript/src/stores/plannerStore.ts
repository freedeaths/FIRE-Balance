...
import { PlannerStage, DEFAULT_USER_PROFILE, DEFAULT_SIMULATION_SETTINGS } from '../types';
import type {
  PlannerData,
  UserProfile,
  IncomeExpenseItem,
...
const createInitialPlannerData = (language: LanguageCode = 'en'): PlannerData => ({
  ...
  const storeCreator = (set: any, get: any) => ({
          ...
          removeIncomeItem: (id: string) => {
            set((state) => ({
              data: {
                ...state.data,
                income_items: state.data.income_items.filter(item => item.id !== id),
                updated_at: new Date().toISOString(),
              },
              isDirty: true,
            }), false, 'removeIncomeItem');
          ...
          removeExpenseItem: (id: string) => {
            set((state) => ({
              data: {
                ...state.data,
                expense_items: state.data.expense_items.filter(item => item.id !== id),
                updated_at: new Date().toISOString(),
              },
              isDirty: true,
            }), false, 'removeExpenseItem');
          ...
          cleanupOrphanedOverrides: (): void => {
            set((state: PlannerState) => {
              const allItems = [...state.data.income_items, ...state.data.expense_items];
              const validItemIds = new Set(allItems.map(item => item.id));


              // Only keep overrides for items that still exist
              const validOverrides = state.data.overrides.filter((override: Override) =>
                validItemIds.has(override.item_id)
              );

              const removedCount = state.data.overrides.length - validOverrides.length;
              ...
              return state;
            }, false, 'cleanupOrphanedOverrides');
          },
          ...
          updateSimulationSettings: (settings: Partial<SimulationSettings>) => {
            set((state) => ({
              data: {
                ...state.data,
                simulation_settings: { ...state.data.simulation_settings, ...settings },
                updated_at: new Date().toISOString(),
              },
              isDirty: true,
            }), false, 'updateSimulationSettings');
          },
          ...
          importConfig: (config: any): boolean => {
            try {
              // 验证导入数据是否合法
              if (!config || !config.version || (!config.user_profile && !config.profile)) {
              ...
              set({
                ...
                data: {
                  ...
                  simulation_settings: config.simulation_settings || DEFAULT_SIMULATION_SETTINGS,
...
export const useUserProfile = () => usePlannerStore((state) => state.data.user_profile);
export const useIncomeItems = () => usePlannerStore((state) => state.data.income_items);
export const useExpenseItems = () => usePlannerStore((state) => state.data.expense_items);
export const useOverrides = () => usePlannerStore((state) => state.data.overrides);
export const useResults = () => usePlannerStore((state) => state.data.results);
export const useSimulationSettings = () => usePlannerStore((state) => state.data.simulation_settings);

// Stage selectors
export const useCurrentStage = () => usePlannerStore((state) => state.currentStage);
export const useIsTransitioning = () => usePlannerStore((state) => state.isTransitioning);
export const useStageProgress = () => usePlannerStore((state) => state.stageProgress);

// Calculation selectors
export const useIsCalculating = () => usePlannerStore((state) => state.isCalculating);
export const useCalculationProgress = () => usePlannerStore((state) => state.calculationProgress);
