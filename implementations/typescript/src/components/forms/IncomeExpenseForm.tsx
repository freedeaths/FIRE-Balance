...
export function IncomeExpenseForm({
  ...
  const [editingItem, setEditingItem] = useState<UIIncomeExpenseItem | null>(null);
  ...
  const coreItems = type === 'income' ? plannerStore.data.income_items : plannerStore.data.expense_items;
  ...
  const removeItem = type === 'income' ? plannerStore.removeIncomeItem : plannerStore.removeExpenseItem;

  // Get templates
  const availableTemplates = templates || (type === 'income' ? DEFAULT_INCOME_TEMPLATES : DEFAULT_EXPENSE_TEMPLATES);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.after_tax_amount_per_period || formData.after_tax_amount_per_period <= 0) {
    ...
    if (formData.frequency !== 'one_time') {
      ...
      if (formData.start_age && formData.end_age && formData.start_age >= formData.end_age) {
  ...
  const handleDelete = (item: IncomeExpenseItem) => {
    removeItem(item.id);
  };
  ...
  const handleSave = () => {
    ...
    const completeItem: IncomeExpenseItem = {
  ...
  const handleFieldChange = (field: keyof IncomeExpenseItem, value: any) => {
  ...
  return (
    <Card>
              ...
              <Table.Tbody>
                {items.map((item, index) => (
                  <Table.Tr key={item.id}>
                    ...
                    <Table.Td>
                      <Group gap="xs">
                        <ActionIcon size="sm" variant="subtle" onClick={() => handleEdit(item)}>
                          <IconEdit size={16} />
                        </ActionIcon>
      ...
      <Modal
        ...
        title={editingItem ? `${t('edit')} ${type} item` : t(`add_${type}_item`)}
        size="lg"
      >
        <Stack gap="md">
          <Grid>
            ...
            <Grid.Col span={{ base: 12, sm: 6 }}>
              <FormField
                ...
                onChange={(value) => handleFieldChange('after_tax_amount_per_period', value)}
              />
            </Grid.Col>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {canUseTemplates ? (
                  <Menu>
                    ...
                    <Menu.Dropdown>
                      {availableTemplates.map((template, index) => (
                        ...
                        >
                          {(() => {
                            const templateKey = template.name?.toLowerCase()
                  ...
                  <>
                    ...
                    <Text size="xs" c="orange" style={{ whiteSpace: 'nowrap', lineHeight: 1.3 }}>
