...
export function MonteCarloResultsChart({
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const riskLevel = data.value >= 0 ? t('low_risk') :
                       data.value >= -500000 ? t('moderate_risk') :
                       data.value >= -1000000 ? t('high_risk') : t('very_high_risk');
      ...
      return (
        <div style={{
          ...
          <p style={{ margin: 0, fontWeight: 600, fontSize: '14px', marginBottom: '8px' }}>
          ...
          <p style={{ margin: 0, fontSize: '11px', color: getColor(data.value), fontWeight: 500 }}>
            {t('risk_level')}: {riskLevel}
          </p>
          <p style={{ margin: 0, fontSize: '11px', color: '#666', marginTop: '6px' }}>
  ...
  return (
    ...
    >
      {({ height: adjustedHeight }) => <ChartContent adjustedHeight={adjustedHeight} distributionData={distributionData} formatCurrency={formatCurrency} CustomTooltip={CustomTooltip} t={t} />}
    </ResponsiveFullscreenChartWrapper>
  );
}
...
function ChartContent({
  ...
  return (
    <div style={{ width: '100%', background: '#fafafa', borderRadius: '8px', padding: isMobilePortrait ? '8px' : '16px' }}>
      <div style={{
      ...
      <ResponsiveContainer width="100%" height={adjustedHeight}>
        ...
        >
          ...
          <XAxis
            ...
            scale="linear"
            ticks={isMobilePortrait ? [5, 25, 50, 75, 95] : [0, 25, 50, 75, 100]}
            height={35}
            interval={0}
          />
          ...
          {!isMobilePortrait && (
            <>
              <ReferenceLine
                ...
                strokeDasharray="3 3"
                strokeWidth={1}
                label={{ value: "5%", position: "top", fontSize: 10, fill: "#ef4444" }}
              />
              <ReferenceLine
                x={25}
                stroke="#f97316"
                strokeDasharray="3 3"
                strokeWidth={1}
                label={{ value: "25%", position: "top", fontSize: 10, fill: "#f97316" }}
              />
            </>
          )}
          ...
          <ReferenceLine
            ...
            strokeDasharray="5 5"
            strokeWidth={2}
            label={!isMobilePortrait ? { value: "50%", position: "top", fontSize: 11, fill: "#3b82f6", fontWeight: "bold" } : undefined}
          />
          ...
          {!isMobilePortrait && (
            <>
              <ReferenceLine
                ...
                strokeDasharray="3 3"
                strokeWidth={1}
                label={{ value: "75%", position: "top", fontSize: 10, fill: "#10b981" }}
              />
              <ReferenceLine
                x={95}
                stroke="#22c55e"
                strokeDasharray="3 3"
                strokeWidth={1}
                label={{ value: "95%", position: "top", fontSize: 10, fill: "#22c55e" }}
              />
            </>
          )}
      ...
      {!isMobilePortrait && (
        <div style={{
          ...
          <div style={{
            ...
            • {t('chart_x_axis_explanation')}<br/>
            • {t('chart_y_axis_explanation')}<br/>
            • {t('chart_reference_lines_explanation')}<br/>
            • {t('chart_risk_color_explanation')}
          </div>
        </div>
      )}
    </div>
