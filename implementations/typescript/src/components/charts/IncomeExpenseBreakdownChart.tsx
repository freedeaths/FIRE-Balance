...
const IncomeExpenseBreakdownChart = React.memo(function IncomeExpenseBreakdownChart({
  height = 400,
  title = '',
  ...
  // i18n
  const i18n = getI18n();
  const t = useCallback((key: string, variables?: Record<string, any>) => i18n.t(key, variables), [i18n]);
    ...
    for (let age = startAge; age <= endAge; age++) {
      ...
      allItems.forEach(item => {
        if (age >= item.start_age && age <= (item.end_age || 999)) {
          ...
          // 支出项目设为负值，收入项目保持正值
          row[item.id] = isIncomeItem ? Math.round(currentAmount) : -Math.round(currentAmount);
        } else {
          row[item.id] = 0;
        }
      });
    ...
    return baseProjectionData.map(row => {
      ...
      overrides.forEach(override => {
        if (override.age === row.age) {
          ...
          const isIncomeItem = incomeItems.some(inc => inc.id === override.item_id);
          ...
          {payload.map((entry: any, index: number) => {
            ...
            return (
              <div key={index} style={{
                ...
                {value >= 0 ? '📈' : '📉'} {displayName}: {formatCurrency(Math.abs(value))}
              </div>
            );
          })}
        </div>
  ...
  return (
    ...
    >
      {({ height: adjustedHeight }) => <ChartContent adjustedHeight={adjustedHeight} title={title} />}
    </ResponsiveFullscreenChartWrapper>
  );
});

// 图表内容子组件 - 使用 mobile display context
const ChartContent = React.memo(function ChartContent({ adjustedHeight, title }: { adjustedHeight: number; title?: string }) {
